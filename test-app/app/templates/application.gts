...
// Full Choices 11 options: pass via @config`;
  ...
  onMultiAdd = (detail: EventChoice) => {
    this.pushEventLog(
      'multi',
      'add',
      `onAdd → value=${String(detail.value)} label=${String(detail.label ?? '')}`,
    );
  };
  ...
  onMultiRemove = (detail: EventChoice) => {
    this.pushEventLog(
      'multi',
      'remove',
      `onRemove → value=${String(detail.value)} label=${String(detail.label ?? '')}`,
    );
  };
  ...
  onTagAdd = (detail: EventChoice) => {
    this.pushEventLog(
      'tags',
      'add',
      `onAdd → value=${String(detail.value)}`,
    );
  };
  ...
  onTagRemove = (detail: EventChoice) => {
    this.pushEventLog(
      'tags',
      'remove',
      `onRemove → value=${String(detail.value)}`,
    );
  };
  ...
  onEventsAdd = (detail: EventChoice) => {
    this.pushEventsDemoLog(
      'add',
      `onAdd → value=${String(detail.value)} label=${String(detail.label ?? '')}  ← side effects only`,
    );
  };
  ...
  onEventsRemove = (detail: EventChoice) => {
    this.pushEventsDemoLog(
      'remove',
      `onRemove → value=${String(detail.value)} label=${String(detail.label ?? '')}  ← side effects only`,
    );
  };
    ...
    >
      ...
      >
        <div class='min-w-0'>
          ...
          <p class='text-xs opacity-60'>Live demos + recipes — not a mystery gallery</p>
        </div>
    ...
    <main class='p-6 max-w-3xl mx-auto space-y-10'>
          ...
          {{#if this.multiLog.length}}
            ...
            >
              {{#each this.multiLog as |line|}}
                <li>
                  <span class='badge badge-xs mr-1 {{line.badgeClass}}'>{{line.kind}}</span>
                  {{line.summary}}
                </li>
              {{/each}}
            </ul>
          {{else}}
            <p class='text-xs opacity-50'>Select or remove cities — events appear here.</p>
          {{/if}}
        </div>
      </CookbookSection>
      ...
      >
        ...
        <div class='space-y-2'>
          ...
          {{else}}
            <p class='text-xs opacity-50'>Add or remove tags — events appear here.</p>
          {{/if}}
        </div>
      </CookbookSection>
      ...
      >
        ...
        <div class='space-y-2'>
          ...
          {{else}}
            <p class='text-xs opacity-50'>Add or remove cities — events appear here.</p>
          {{/if}}
        </div>
      </CookbookSection>
      ...
      >
        ...
        <div class='space-y-2'>
          <p class='text-sm font-medium'>Multi + custom remove
